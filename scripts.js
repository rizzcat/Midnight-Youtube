...
class ParticleEngine {
    constructor() {
        ...
        if (!this.enabled) { this.container.style.display = "none"; return; }
        this.particles = [];
        this.max = 24;
        this.timer = setInterval(() => this.spawn(), 2200);
    }
    spawn() {
        ...
        p.style.cssText = `position:absolute;left:${x}%;bottom:-10px;width:${size}px;height:${size}px;border-radius:50%;background:var(--accent);opacity:0;pointer-events:none;animation:particleFloat ${duration}s ${delay}s ease-in-out forwards;box-shadow:0 0 ${size*2}px var(--accent);`;
